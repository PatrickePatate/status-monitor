...
use App\Filament\Resources\Checks\DnsCheckResource\Pages\CreateDnsCheck;
...
class DnsCheckTest extends TestCase
{
    ...
    public function test_dns_checks_can_be_created(): void
    {
        $service = Service::factory()->count(1)->create()->first();

        $before = DnsCheck::count();
        $check = DnsCheck::factory()->count(1)->create(['service_id' => $service->id])->first();
        $after = DnsCheck::count();
...
//
    ...
    public function test_admin_can_create_dns_tests_via_dashboard(){
        $service = Service::factory()->count(1)->create()->first();
        $this->actingAs(User::first());
        Livewire::test(CreateDnsCheck::class)
            ->fillForm([
                'domain' => 'lucienpuget.fr',
                'service_id' => $service->id,
                'ipv4_match' => null,
                'ipv6_match' => null,
                'metric_id' => null,
